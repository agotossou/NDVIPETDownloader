...
# ===========================
safe_download <- function(url, destfile, attempts=8, wait_sec=10) {
  for(i in seq_len(attempts)){
    success <- tryCatch({
      download.file(url, destfile, mode="wb", quiet=TRUE)
      if(!file.exists(destfile) || file.info(destfile)$size==0) stop("File empty")
      TRUE
    }, error=function(e){ FALSE })
    if(isTRUE(success)) return(TRUE)
    Sys.sleep(wait_sec)
  }
  return(FALSE)
}
...
server <- function(input, output, session){
  ...
  observeEvent(input$download_pentad,{
    ...
    withProgress(message="Downloading Pentad files...", value=0,{
      ...
      for(i in seq_along(files)){
        ...
        if(action=="download"){
          success <- safe_download(url, destfile, attempts=8, wait_sec=10)
          if(success){appendLog_pentad(paste("Downloaded:",fname)); downloaded <- downloaded+1} else {appendLog_pentad(paste("Failed:",fname)); failed <- failed+1}
  ...
  observeEvent(input$download_dekad,{
    ...
    if(is.null(outdir)||outdir==""||!dir.exists(outdir)){validation_msg_dekad("Output directory does not exist."); return()} else validation_msg_dekad(NULL)
    ...
    withProgress(message="Downloading Dekad files...", value=0,{
      ...
      for(i in seq_along(files)){
        ...
        if(action=="download"){
          success <- safe_download(url, destfile, attempts=8, wait_sec=10)
          if(success){appendLog_dekad(paste("Downloaded:",fname)); downloaded <- downloaded+1} else {appendLog_dekad(paste("Failed:",fname)); failed <- failed+1}
  ...
  output$preview_text_pet <- renderUI({
    ...
    } else {tags$div(tags$strong("Dataset: "),"PET Daily",tags$br(),tags$strong("First available: "),sprintf("%02d-%02d-%04d",ar$first$day,ar$first$month,ar$first$year),tags$br(),tags$strong("Latest available: "),sprintf("%02d-%02d-%04d",ar$last$day,ar$last$month,ar$last$year))}
  })
  ...
  validate_inputs_pet <- function(){
    sy <- input$start_year_pet; sm <- input$start_month_pet; sd <- input$start_day_pet
    ey <- input$end_year_pet; em <- input$end_month_pet; ed <- input$end_day_pet
    cur_date <- Sys.Date()
    if(!is.numeric(sy) || !is.numeric(sm) || !is.numeric(sd) || !is.numeric(ey) || !is.numeric(em) || !is.numeric(ed)) return("Year, month or day inputs must be numeric.")
    if(sm < 1 || sm > 12 || em < 1 || em > 12 || sd < 1 || sd > 31 || ed < 1 || ed > 31) return("Month/day must be valid.")
    if(sy < 2001 || sy > 2050 || ey < 2001 || ey > 2050) return("Year must be between 2001 and 2050.")
    if(!is_before_or_equal(sy,sm,sd,ey,em,ed)) return("Start date must be earlier than or equal to end date.")
    end_date <- as.Date(sprintf("%04d-%02d-%02d",ey,em,ed))
    if(end_date>cur_date) return("Cannot select future dates.")
    return(NULL)
  }
  ...
  observeEvent(input$download_pet,{
    ...
    if(is.null(outdir)||outdir==""||!dir.exists(outdir)){validation_msg_pet("Output directory does not exist."); return()} else validation_msg_pet(NULL)
    ...
    withProgress(message="Downloading PET Daily files...", value=0,{
      ...
      for(i in seq_along(files)){
        ...
        if(action=="download"){
          success <- safe_download(url, destfile, attempts=8, wait_sec=10)
          if(success){appendLog_pet(paste("Downloaded:",fname)); downloaded <- downloaded+1} else {appendLog_pet(paste("Failed:",fname)); failed <- failed+1}
